...
const LessonArea: React.FC<LessonAreaProps> = ({ category, lesson, onBack, onComplete }) => {
    ...
    const renderHeader = () => (
        <header className="bg-white/95 backdrop-blur-md border-b border-slate-200/60 z-50 sticky top-0">
            <div className="max-w-7xl mx-auto px-3 sm:px-4 h-14 sm:h-20 flex items-center justify-between">
                ...
                {/* Progress Bar e Fullscreen - Mobile e Desktop */}
                <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
                    ...
                    >
                        {isFullscreen ? (
                            <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 14h4v4M4 14l5 5M20 10h-4V6M20 10l-5-5M4 10h4V6M4 10l5-5M20 14h-4v4M20 14l-5 5" />
                            </svg>
                        ) : (
                            <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="hidden sm:block w-16 sm:w-48 h-1.5 sm:h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div
                            className="h-full bg-gradient-to-r from-brand-500 to-brand-600 transition-all duration-500"
                            style={{ width: `${progressPercentage}%` }}
                        />
                    </div>
                    <span className="hidden sm:inline text-xs sm:text-sm font-bold text-slate-700">{progressPercentage}%</span>
                </div>
            ...
            <div className="h-full flex flex-col lg:flex-row gap-0 bg-slate-200 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col min-h-0 items-center justify-center p-4 lg:p-8 relative">
                    ...
                    <div className="w-full max-w-[1280px] aspect-video bg-white shadow-2xl rounded-sm border border-slate-300 relative flex flex-col animate-fadeIn select-none transform transition-transform duration-300">
                        {/* Indicativo de Slide (Flutuante Mobile) */}
                        {!isFullscreen && (
                            <div className="md:hidden absolute -top-8 left-0 z-50 text-xs font-bold text-slate-500 bg-white/50 px-3 py-1 rounded-full backdrop-blur-sm whitespace-nowrap">
                    ...
                    <div className="w-full max-w-[1280px] mt-4 flex items-center justify-center md:justify-between px-2 gap-4">
                        <div className="hidden md:block text-sm font-medium text-slate-600 bg-white/50 px-3 py-1 rounded-full backdrop-blur-sm whitespace-nowrap flex-shrink-0">
                            Slide {currentSlideIndex + 1} de {lesson.slides.length}
                        </div>
    ...
    return (
        <div ref={lessonContainerRef} className={`flex flex-col bg-slate-50 font-sans overflow-hidden ${isFullscreen ? 'h-screen w-screen fixed inset-0 z-[100]' : 'h-screen'}`}>
            {renderHeader()}
