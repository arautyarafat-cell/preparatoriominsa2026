    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans selection:bg-indigo-100">
      ...
      <div className="bg-white px-6 py-4 flex items-center justify-between shadow-sm sticky top-0 z-10">
        ...
        <h1 className="text-sm font-bold text-slate-900 uppercase tracking-widest">
          Decifre o Termo {termQueue.length > 0 && <span className="text-xs text-indigo-500 ml-2">({termQueue.length} restantes)</span>}
        </h1>
        <div className="flex gap-1">
          {Array.from({ length: 5 }).map((_, i) => (
            <span key={i} className={`text-2xl transition-all ${i < lives ? 'text-red-500 scale-100' : 'text-slate-200 scale-75 grayscale'}`}>
              ❤️
            </span>
          ))}
        </div>
